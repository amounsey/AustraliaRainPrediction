...
###Logistic Model
The functional for the 

\begin{equation}
\begin{gathered}
ln\frac{p(x_i)}{1-p(x_i)}=\sum_{j=0}^{M}b_jx_{i,j} \\
\\
\text{alternatively in vector notation}\\
\\
ln\frac{p(\mathbf{x_i};\mathbf{\beta})}{1-p(\mathbf{x_i};\beta)}=\mathbf{x_i}\beta 
\end{gathered}
(\#eq:logistic)
\end{equation}
...
> $x_{i,j}$ represents the $i^{th}$ observation feature $j$, $\forall \; j= 1,2,...M$
...
\begin{equation}
p(\mathbf{x_i};\mathbf{\beta})=\frac{e^{\mathbf{x_i}\beta}}{1+e^{\mathbf{x_i}\beta}}=\frac{1}{1+e^{-\mathbf{x_i}\beta}}
(\#eq:logistic1)
\end{equation}

Equation \@ref(eq:logistic1) is a logistic function 
